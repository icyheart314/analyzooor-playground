...
BEGIN
  ...
  WITH token_activity AS (
    SELECT 
      COALESCE(s.input_token_mint, s.output_token_mint) as token_mint,
      COUNT(*) as activity_count
    FROM swaps s
    WHERE s."timestamp" >= time_threshold
      AND (s.input_token_mint IS NOT NULL OR s.output_token_mint IS NOT NULL)
    GROUP BY COALESCE(s.input_token_mint, s.output_token_mint)
